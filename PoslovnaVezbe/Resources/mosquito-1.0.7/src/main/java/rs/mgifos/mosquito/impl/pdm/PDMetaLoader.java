...
public class PDMetaLoader implements IMetaLoader {
	...
	class PDMColumn extends PDMElement {
		...
		protected boolean cannotModify = false;
		...
		protected MetaColumn getCreateMetaColumn() throws SAXException {
			...
			if ((retVal = getMetaColumnByPDMColId(id)) == null) {
				...
				retVal = new MetaColumn(this.code, this.name, this.parentTable,
						...
						comment, this.format, this.minValue, this.maxValue, this.cannotModify);
	...
	class XmlElem {
		...
		public final static String A_FORMAT = "a:Format";
		
		public final static String A_LOW_VALUE = "a:LowValue";
		
		public final static String A_HIGH_VALUE = "a:HighValue";
		
		public final static String A_CANNOT_MODIFY = "a:CannotModify";
		...
		public boolean isACannotModify() {
			return 	A_CANNOT_MODIFY.equals(qName);
		}
		
		public boolean isAFormat() {
			return 	A_FORMAT.equals(qName);
		}
		
		public boolean isALowValue() {
			return 	A_LOW_VALUE.equals(qName);
		}
		
		public boolean isAHighValue() {
			return 	A_HIGH_VALUE.equals(qName);	
		}
	...
	class XmlHandlerDelegate extends DefaultHandler {
		...
		public void characters(char[] ch, int start, int length)
			...
			} else if(actualXE.isALowValue()) {
				// __________________________________________________A_LOW_VALUE
			    if(prevActualXE.isOColumn()) {
			        actualColumn.minValue = sCh;
			    }
			}else if(actualXE.isACannotModify()) {
				// __________________________________________________A_CANNOT_MODIFY
			    if(prevActualXE.isOColumn()) {
					if(sCh.equals("1")) {
						actualColumn.cannotModify = true;
					}
			    }
			}else if(actualXE.isAHighValue()) {
				// __________________________________________________A_HIGH_VALUE
			    if(prevActualXE.isOColumn()) {
			        actualColumn.maxValue = sCh;
			    }
			}else if(actualXE.isAFormat()) {
				// __________________________________________________A_FORMAT
			    if(prevActualXE.isOColumn()) {
			        actualColumn.format += sCh;
			    }
		...
		public void startElement(String uri, String localName, String qName,
			...
			if (qName.equals(XmlElem.A_NAME)) {
				...
				actualXE = new XmlElem(qName, null, null);
			} else if(qName.equals(XmlElem.A_LOW_VALUE)) {
			        actualXE = new XmlElem(qName, null, null);
			} else if(qName.equals(XmlElem.A_CANNOT_MODIFY)) {
			        actualXE = new XmlElem(qName, null, null);
			} else if(qName.equals(XmlElem.A_HIGH_VALUE)) {
			        actualXE = new XmlElem(qName, null, null);
			} else if(qName.equals(XmlElem.A_FORMAT)) {
			        actualXE = new XmlElem(qName, null, null);
